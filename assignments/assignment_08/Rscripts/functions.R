...
build_asv_table <- function(metadata, # metadata object for multi-seq-run samples; must contain "run" column and fwd/rev filepath columns
  ...
  saveRDS(seqtab.nochim,paste0(asv.table.dir,"/16S_V3V4_seqtab_nochim.RDS"))
  # TRACK READS ####
  ...
  if(any(metadata[["control"]]) & sum(seqtab.nochim[metadata[["control"]],]) > 0){
    ...
    print(paste0(sum(contams$contaminant)," likely contaminant(s) were removed from the ASV table of Run ",as.character(run.id),"."))
  }
